...
class AI_Interface:
    def __init__(self):
		...
		<rules>
			...
			<3>Если все поля в "data" тебе удалось заполнить то запиши "success" в "question"</3>
			<4>Нигде не изменяй уже заполенные поля"</4>
			<5>Не нужно вставлять в вопрос пользователю описание колонки из таблицы 1 в 1</5>
		</rules>
		<input>
			{
		...
		<output>
			{
				...
				"question": "новый вопрос который ты cгенерировал в соответствии с текущим состоянием таблицы либо success когда в data уже все заполнено",
			}
		</output>
		"""
